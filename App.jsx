  ...
  const VisitForm = () => {
    ...
    return (
      <div className="pb-24">
        ...
        <div className="max-w-2xl mx-auto px-4 pt-4">
          <label className="block mb-3">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1 block">Customer *</span>
            <select value={f.customerId} onChange={e => upd("customerId", e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-slate-800 text-base focus:outline-none focus:ring-2 focus:ring-teal-400">
            ...
            {data.customers.length === 0 && (
              <button type="button" onClick={() => go("customerForm", { _back: screen })}
                className="mt-2 text-sm text-teal-600 font-medium flex items-center gap-1">
                <Icons.plus size={14} /> Create a customer first
              </button>
            )}
          </label>
